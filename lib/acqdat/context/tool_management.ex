defmodule Acqdat.Context.ToolManagement do
  ...
  def tool_transaction(params) do
    %{transaction: transaction_type, tool_ids: tool_uuids,
  ...
  defp run_transaction(tool_manifest, status = "issue") do
    ...
    case Tool.get_all_by_uuids_and_status(tool_uuids, @in_inventory_status) do
      ...
      tool_ids ->
        issue_list = tool_manifest |> Map.replace!(:tool_ids, tool_ids) |> tool_insert_manifest(status)
  ...
  defp run_transaction(tool_manifest, status = "return") do
    ...
    case Tool.get_all_by_uuids_and_status(tool_uuids, @issued_status) do
      ...
      tool_ids ->
        return_list = tool_manifest |> Map.replace!(:tool_ids, tool_ids) |> tool_insert_manifest(status)
